...
// ---------------- Stooq CSV (Forex / Stocks / Commodities, daily+weekly) ----------------
...
class StooqProvider extends MarketDataProvider {
  stooqInterval(timeframe) {
    ...
    return null; // intraday not reliably available on the public CSV endpoint
  }
  async getCandles({ ref, timeframe, limit = 300 }) {
  ...
  async getPrice({ ref }) {
    try {
      const text = await fetchText(`https://stooq.com/q/l/?s=${encodeURIComponent(ref.toLowerCase())}&f=sd2t2ohlcv&h&e=csv`);
      const lines = text.trim().split(/\r?\n/);
