...
export function technicalScore(candles, ind) {
  ...
  const push = (label, side, points, detail) => out.push({ label, side, points, detail });
  const f = (v) => fmtN(v);
  ...
  const r = ind.rsi[i];
  if (r != null) {
    if (r > 55) push(`RSI ${r.toFixed(1)} — bullish momentum`, 'bull', 8, 'Buyers in control above 55.');
    else if (r < 45) push(`RSI ${r.toFixed(1)} — bearish momentum`, 'bear', 8, 'Sellers in control below 45.');
    else push(`RSI ${r.toFixed(1)} — neutral zone`, 'neutral', 0, 'No momentum edge between 45–55.');
    if (r < 30) push('RSI oversold — bounce possible', 'bull', 3, 'Snap-back risk; do not chase shorts.');
    if (r > 70) push('RSI overbought — pullback possible', 'bear', 3, 'Chasing longs here is expensive.');
  }
  if (ind.macdLine[i] != null && ind.signalLine[i] != null) {
    if (ind.macdLine[i] > ind.signalLine[i]) push('MACD above signal — bullish', 'bull', 8, `MACD ${f(ind.macdLine[i])} > signal ${f(ind.signalLine[i])}`);
    else push('MACD below signal — bearish', 'bear', 8, `MACD ${f(ind.macdLine[i])} < signal ${f(ind.signalLine[i])}`);
  }
  if (ind.bbUpper[i] != null && ind.bbLower[i] != null) {
    if (c.close > ind.bbUpper[i]) push('Close above upper band — overextended', 'bear', 4, 'Mean-reversion risk up here.');
    else if (c.close < ind.bbLower[i]) push('Close below lower band — washed out', 'bull', 4, 'Bounce possible, not guaranteed.');
    else push('Price inside Bollinger Bands', 'neutral', 0, 'No band breakout.');
  }
  ...
  if (ind.stDir[i] != null) {
    if (ind.stDir[i] === 1) push('Supertrend bullish', 'bull', 10, 'Price above ATR trailing stop.');
    else push('Supertrend bearish', 'bear', 10, 'Price below ATR trailing stop.');
  }
  ...
  if (bullEng) push('Bullish engulfing candle', 'bull', 6, 'Buyers overwhelmed prior sellers.');
  if (bearEng) push('Bearish engulfing candle', 'bear', 6, 'Sellers overwhelmed prior buyers.');
  if (body / range > 0.7 && c.close > c.open) push('Strong bullish displacement', 'bull', 4, 'Wide-range buying candle.');
  if (body / range > 0.7 && c.close < c.open) push('Strong bearish displacement', 'bear', 4, 'Wide-range selling candle.');
...
export function ictScore(candles, ind, ict) {
  ...
  if (upStruct && !dnStruct) push('Bullish structure (HH/HL)', 'bull', 20, 'Recent swings print higher highs / higher lows.');
  else if (dnStruct && !upStruct) push('Bearish structure (LH/LL)', 'bear', 20, 'Recent swings print lower highs / lower lows.');
  else push('Mixed structure — no stair-step', 'neutral', 0, 'HH/HL and LH/LL both present or absent.');
  ...
  if (bosB && !bosS) push('Recent bullish BOS/CHOCH', 'bull', 15, 'Structure broke upward in the last 8 bars.');
  else if (bosS && !bosB) push('Recent bearish BOS/CHOCH', 'bear', 15, 'Structure broke downward in the last 8 bars.');
  else if (bosB && bosS) push('BOS both ways — chop', 'neutral', 0, 'Conflicting breaks = range, not trend.');
  else push('No recent BOS/CHOCH', 'neutral', 0, 'Structure intact, no fresh break.');
  ...
  if (swB) push('Sell-side liquidity swept', 'bull', 15, 'Stops raided below lows — fuel for longs.');
  if (swS) push('Buy-side liquidity swept', 'bear', 15, 'Stops raided above highs — fuel for shorts.');
  if (!swB && !swS) push('No fresh liquidity sweep', 'neutral', 0, 'No stop-raid in the last 5 bars.');
  ...
  if (fB && fS) push('FVGs both sides — magnet chop', 'neutral', 0, 'Price has unfinished business up and down.');
  else if (fB) push('Unfilled bullish FVG', 'bull', 10, 'Imbalance below that price may revisit and bounce.');
  else if (fS) push('Unfilled bearish FVG', 'bear', 10, 'Imbalance above that price may revisit and reject.');
  else push('No open FVG nearby', 'neutral', 0, 'Gaps already filled or none formed.');
  ...
  if (bullOB && !bearOB) push('Active bullish order block below', 'bull', 15, `${bullOB.label} — demand shelf under price.`);
  else if (bearOB && !bullOB) push('Active bearish order block above', 'bear', 15, `${bearOB.label} — supply shelf over price.`);
  else push('No one-sided active order block', 'neutral', 0, 'No fresh shelf backing either side.');

  // 6) Premium / discount (+10)
  if (ict.zone && ict.zone.startsWith('Discount')) push('Price in discount', 'bull', 10, 'Lower third of the dealing range — longs pay less.');
  else if (ict.zone && ict.zone.startsWith('Premium')) push('Price in premium', 'bear', 10, 'Upper third of the range — shorts get better location.');
  else push('Price at equilibrium', 'neutral', 0, 'Mid-range — worst place to chase.');
  ...
  if (dB && !dS) push('Recent bullish displacement', 'bull', 10, 'Wide-range buying shows initiative.');
  else if (dS && !dB) push('Recent bearish displacement', 'bear', 10, 'Wide-range selling shows initiative.');
  else push('No one-sided displacement', 'neutral', 0, 'No initiative candle in the last 5 bars.');

  // 8) Session (+5)
  const sess = ict.session;
  if (sess?.killzone) push(`${sess.name} killzone active`, sess.bias === 1 ? 'bull' : sess.bias === -1 ? 'bear' : 'neutral', sess.bias === 0 ? 0 : 5, 'Moves made in killzones are more trustworthy.');
  else push(sess ? `${sess.name} session — off killzone` : 'Session unknown', 'neutral', 0, 'Patience: wait for London/NY windows.');

  // EMA trend gate (no points, context only)
  if (ind?.ema50?.[n - 1] != null) {
    out.push({ label: c.close > ind.ema50[n - 1] ? 'Price holds above EMA 50' : 'Price capped below EMA 50', side: 'neutral', points: 0, detail: 'Context gate for ICT entries.' });
  }
  return finishScore(out);
}
